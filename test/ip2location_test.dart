...
void main() {
  ...
  group('Test Individual Fields', () {
    test('Test Country Short', () async {
      ...
      expect(result.ipAddress, '8.8.8.8');
      expect(result.countryShort, 'US');
      expect(result.countryLong, 'Not_Supported');
      expect(result.region, 'Not_Supported');
      expect(result.city, 'Not_Supported');
      expect(result.isp, 'Not_Supported');
      expect(result.latitude?.toStringAsFixed(6), '0.000000');
      expect(result.longitude?.toStringAsFixed(6), '0.000000');
      expect(result.domain, 'Not_Supported');
      expect(result.zipCode, 'Not_Supported');
      expect(result.netSpeed, 'Not_Supported');
      expect(result.timeZone, 'Not_Supported');
      expect(result.iddCode, 'Not_Supported');
      expect(result.areaCode, 'Not_Supported');
      expect(result.weatherStationCode, 'Not_Supported');
      expect(result.weatherStationName, 'Not_Supported');
      expect(result.mcc, 'Not_Supported');
      expect(result.mnc, 'Not_Supported');
      expect(result.mobileBrand, 'Not_Supported');
      expect(result.elevation, 0.0);
      expect(result.usageType, 'Not_Supported');
      expect(result.addressType, 'Not_Supported');
      expect(result.category, 'Not_Supported');
      expect(result.district, 'Not_Supported');
      expect(result.asn, 'Not_Supported');
      expect(result.asName, 'Not_Supported');
    });
    test('Test Country Long', () async {
      var ipl = IP2Location(databasePath: db26IPv4File);
      var result = await ipl.getCountryLong('8.8.8.8');
      expect(result.ipAddress, '8.8.8.8');
      expect(result.countryShort, 'Not_Supported');
      expect(result.countryLong, 'United States of America');
      expect(result.region, 'Not_Supported');
      expect(result.city, 'Not_Supported');
      expect(result.isp, 'Not_Supported');
